		...
		// looking for sliding panel
			...
			if (function_exists('GSPB_make_dynamic_link') && !empty($block['attrs']['dynamicEnable'])) {
				...
				if ($repeaterField && !empty($block['attrs']['repeaterArray'][$repeaterField])) {
					...
					if($replaced){
						$html = preg_replace('/href\s*=\s*"([^"]*)"/i', 'href="' . $replaced . '"', $html);
					}
				} else {
			...
			if (function_exists('GSPB_make_dynamic_flatvalue') && !empty($block['attrs']['dynamicMetas']['buttonContent']['dynamicEnable'])) {
				...
				if ($repeaterfield && !empty($block['attrs']['dynamicMetas']['buttonContent']['repeaterArray'][$repeaterfield])) {
					$replaced = GSPB_get_value_from_array_field($repeaterField, $block['attrs']['dynamicMetas']['buttonContent']['repeaterArray']);
					$replaced = GSPB_field_array_to_value($replaced, ', ');
...
function gspb_greenShift_editor_assets()
{
	...
	$global_styles = (!empty($sitesettings['global_styles'])) ? $sitesettings['global_styles'] : '';
	...
	wp_localize_script(
		...
		array(
			...
			'global_styles' => apply_filters('gspb_global_styles', $global_styles),
		)
	);
...
function gspb_global_variables()
{
	...
	if (!is_admin()) {
		...
		if($stylesrender){
			wp_register_style('greenshift-reusable-css', false);
			wp_enqueue_style('greenshift-reusable-css');
			wp_add_inline_style('greenshift-reusable-css', $stylesrender);
		}
